...
fileprivate struct OctreeNode {
    ...
    func merge(tree: Octree, coord: OctreeCoordinate) -> OctreeNode {
        ...
        let neighbours = DualMarchingCuboids.findCellCorners(grid: tree.grid, x: coord.x, y: coord.y, z: coord.z, index: index, width: coord.nodeSize, height: coord.nodeSize, depth: coord.nodeSize)
